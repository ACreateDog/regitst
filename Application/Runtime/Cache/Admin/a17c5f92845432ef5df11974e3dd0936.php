 ...
 list($first, $last) = explode(' / ',$route); foreach( $nav as $key => $item ){ if($item['name'] == $first){ if(!$item['url']){ echo '<li class="active open">'; }else{ echo '<li class="active">'; } }else{ echo '<li>'; } if($item['url']){ echo '<a href="'.$item['url'].'">'; }else{ echo '<a href="#" class="menu-dropdown">'; } echo '<i class="menu-icon '.$item['ico'].'"></i>'; echo '<span class="menu-text">'.$item['name'].'</span>'; if(!$item['url']){ echo '<i class="menu-expand"></i>'; } echo '</a>'; if(!$item['url']){ echo '<ul class="submenu">'; foreach( $item['children'] as $k => $value ){ if($value['name'] == $last){ echo '<li class="active">'; }else{ echo '<li>'; } echo '<a href="'. $value['url'] .'">'; echo '<span class="menu-text">'.$value['name'].'</span>'; echo '</a></li>'; } echo '</ul>'; } } ?>
    ...
    <div class="widget">
        ...
        <div class="widget-body">
            <div class="btn-group">
                <a type="button" id="export" class="btn btn-default" href="<?php echo U('Enter/civil_studnet_info_export');?>?type=1">
            ...
            <table class="table table-striped table-hover table-bordered" id="editabledatatable">
                ...
                <tbody>
                    <?php if(is_array($data)): foreach($data as $k=>$vo): ?><tr>
                            ...
                            <td class="center ">
                                <?php if($vo["civil_status"] == 1): ?>派中
                                    <?php elseif($vo["civil_status"] == 2): ?>已录取
                                    <?php else: ?>未录取<?php endif; ?>
                            </td>
                            <td registerid="<?php echo ($vo["register_id"]); ?>">
                                <?php if($vo["civil_status"] == 1): ?><a href="javascript:void(0)" select_id="<?php echo ($vo["register_id"]); ?>" class="btn btn-success btn-xs" onclick="adopt(this)"><i class="fa fa-check"></i> 录取</a>
                                    ...
                                    <?php elseif($vo["civil_status"] == 2): ?>
                                    <a href="javascript:void(0)" select_id="<?php echo ($vo["register_id"]); ?>" class="btn btn-success btn-xs" onclick="adopt(this)" disabled="true"><i class="fa fa-check"></i> 录取</a>
                                    <a href="javascript:void(0)" select_id="<?php echo ($vo["register_id"]); ?>" class="btn btn-danger btn-xs" onclick="refuse(this)"><i class="fa fa-times"></i> 拒绝</a>
                                    <?php else: ?>
                                    ...
                                    <a href="javascript:void(0)" select_id="<?php echo ($vo["register_id"]); ?>" class="btn btn-danger btn-xs" onclick="refuse(this)" disabled="true"><i class="fa fa-times"></i> 拒绝</a><?php endif; ?>
                            </td>
...
<script>
    ...
    <script>
        ...
        //发布
        function issued(){
            var url = "<?php echo U('Enter/civil_issued_admis');?>";
        ...
        //录取
        function admis(){
            var url = "<?php echo U('Enter/civil_admis');?>";
            var reid = [];
        ...
        //不录取
        function nadmis(){
            var url = "<?php echo U('Enter/civil_Nadmis');?>";
            var reid = [];
            $("input[name='reid'][disabled!='disabled']").each(function(){
                if(this.checked){
                    reid.push(this.value);
