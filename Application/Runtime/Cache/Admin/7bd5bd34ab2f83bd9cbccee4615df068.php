...
<div class="main-container container-fluid">
 ...
 list($first, $last) = explode(' / ',$route); foreach( $nav as $key => $item ){ if($item['name'] == $first){ if(!$item['url']){ echo '<li class="active open">'; }else{ echo '<li class="active">'; } }else{ echo '<li>'; } if($item['url']){ echo '<a href="'.$item['url'].'">'; }else{ echo '<a href="#" class="menu-dropdown">'; } echo '<i class="menu-icon '.$item['ico'].'"></i>'; echo '<span class="menu-text">'.$item['name'].'</span>'; if(!$item['url']){ echo '<i class="menu-expand"></i>'; } echo '</a>'; if(!$item['url']){ echo '<ul class="submenu">'; foreach( $item['children'] as $k => $value ){ if($value['name'] == $last){ echo '<li class="active">'; }else{ echo '<li>'; } echo '<a href="'. $value['url'] .'">'; echo '<span class="menu-text">'.$value['name'].'</span>'; echo '</a></li>'; } echo '</ul>'; } } ?>
            ...
            <div class="widget">
                ...
                <div class="widget-body">
                    ...
                    <div class="table-toolbar operation">
                        ...
                        <div class="btn-group">
                            <a type="button" id="export" class="btn btn-default" href="<?php echo U('Register/export_excel');?>?type=issue&name=<?php echo ($reminder); ?>">
                                <i class="fa fa-download"></i>导出
                            </a>
                        </div>
                        <?php if((!empty($student_info)) and ($where == 0)): ?><div class="btn-group">
                                <a type="button" id="issue" class="btn btn-default" url="<?php echo U('Register/issue');?>">
                                    <i class="glyphicon glyphicon-volume-up"></i>立即发布
                                </a>
                            </div>
                            <div class="btn-group">
                                <a type="button" id="checkbox_assign" class="btn btn-default">分配学校</a>
                            </div><?php endif; ?>
                    </div>
                    ...
                    <table class="table_data table table-striped table-hover table-bordered" id="student_info">
                        ...
                        <?php if(is_array($student_info)): foreach($student_info as $key=>$vo): ?><tr>
                                ...
                                <td><?php echo ($vo["assign_junior"]); ?></td>
                                <!--<td><?php echo ($vo["civil_status"]); ?></td>-->
                                <!--<td><?php echo ($vo["pub_status"]); ?></td>-->
                                <td><?php echo ($vo["assign_junior_status"]); ?></td>
                                <td><?php echo ($vo["assign_junior_operation"]); ?></td>
                            </tr><?php endforeach; endif; ?>
                        </tbody>
                    </table>
                    <input value="<?php echo U('Register/download_template');?>" id="download-template-url" type="hidden">
                    <input type="hidden" id="present_url" save_volunteer="<?php echo U('save_student_volunteer');?>" value="<?php echo U('assign_junior');?>">
                </div>
            </div>
        </div>
    </div>
    ...
    <input value="<?php echo U('Register/import_assign_junior');?>" id="import-url" type="hidden">
...
<script>
    ...
    <script src="/register_system/Public/admin/student/js/assign_junior.js"></script>
