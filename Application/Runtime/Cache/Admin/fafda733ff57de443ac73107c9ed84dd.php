            ...
            <div class="widget">
                ...
                <div class="widget-body">
                            ...
                            <div class="panel panel-default">
                                ...
                                <div id="collapseOne" class="panel-collapse collapse">
                                    <div class="panel-body">
                                        <label>学生类型</label>
                                        <select id="student_type" data="<?php echo ($where['0']); ?>">
                                            <option value="2">全部</option>
                                            <option value="0">学区生</option>
                                            <option value="1">统筹生</option>
                                        </select>
                                        <label>派位结果</label>
                                        <select id="lottery" data="<?php echo ($where['1']); ?>">
                                            <option value="2">全部</option>
                                            <option value="1">派中</option>
                                            <option value="0">未派中</option>
                                            <!--<option value="-1">等待中</option>-->
                                        </select>
                                        <?php if(($area['school_district'] != null)): ?><label>学区</label>
                                            <select id="school_district" data="<?php echo ($where['2']); ?>">
                                                <option value="0">全部</option>
                                                <?php if(is_array($area['school_district'])): foreach($area['school_district'] as $k=>$vo): ?><option value="<?php echo ($k); ?>"><?php echo ($vo); ?></option><?php endforeach; endif; ?>
                                            </select>
                                            <label>学校</label>
                                            <select id="school" data="<?php echo ($where['3']); ?>">
                                                <option value="0">全部</option>
                                            </select>
                                            <label>班级</label>
                                            <select id="class" data="<?php echo ($where['4']); ?>">
                                                <option value="0">全部</option>
                                            </select>
                                        <?php elseif($area['primary_school'] != null): ?>
                                        <!--<notempty name="area['school_district']">-->
                                            <label>学校</label>
                                            <select id="school" data="<?php echo ($where['3']); ?>">
                                                <option value="0">全部</option>
                                                <?php if(is_array($area['primary_school'])): foreach($area['primary_school'] as $k=>$vo): if(is_array($vo)): foreach($vo as $k1=>$vo1): ?><option value="<?php echo ($k1); ?>"><?php echo ($vo1); ?></option><?php endforeach; endif; endforeach; endif; ?>
                                            </select>
                                            <label>班级</label>
                                            <select id="class" data="<?php echo ($where['4']); ?>">
                                                <option value="0">全部</option>
                                            </select>
                                        <?php elseif($area['class'] != null): ?>
                                            <label>班级</label>
                                            <select id="class" data="<?php echo ($where['4']); ?>">
                                                <option value="0">全部</option>
                                                <?php if(is_array($area['class'])): foreach($area['class'] as $k=>$vo): if(is_array($vo)): foreach($vo as $k1=>$vo1): ?><option value="<?php echo ($k1); ?>"><?php echo ($vo1); ?></option><?php endforeach; endif; endforeach; endif; ?>
                                            </select><?php endif; ?>
                                        <br/><br/>
                                        <label>志愿学校(民办)</label>
                                        <select id="civil_junior" data="<?php echo ($where['5']); ?>">
                                        ...
                                        <label>志愿学校(公办)</label>
                                        <select id="pub_junior" data="<?php echo ($where['6']); ?>">
                                            <option value="-1">全部</option>
                    ...
                    <div class="table-toolbar operation">
                        <div class="btn-group">
                            <a type="button" id="export" class="btn btn-default" href="<?php echo U('Register/export_excel');?>?type=lead&name=信息统计表">
                    ...
                    <input type="hidden" id="present_url" value="<?php echo U('Register/look_student_info');?>">
                    <!--<textarea style="display: none;" id="data_area"><?php echo ($organization); ?></textarea>-->
                    <script>
                        var area = <?php echo ($organization); ?>;//机构框架
                    </script>
                </div>
            </div>
        </div>
...
<script>
    ...
    <script src="/register_system/Public/admin/student/js/look_student_info.js"></script>
